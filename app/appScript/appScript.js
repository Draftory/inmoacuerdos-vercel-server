...
// List of placeholders that should be hidden and not replaced until the user provides input
 ...
 async function updateSpecificPlaceholder(placeholderKey, event) {
  ...
  placeholderElements.forEach((placeholderElement) => {
    if (foundEmptyClause) {
      // Si encontramos una cláusula vacía para la opción seleccionada, ocultamos el placeholder
      placeholderElement.style.display = 'none';
      return;
    }
    // Mostramos el placeholder si tiene contenido
    placeholderElement.style.display = 'inline';
    if (!clauseText && !inputValue) {
      placeholderElement.innerHTML = "_________";
    } else {
      placeholderElement.innerHTML = clauseText || 
    ...
    if (
      ...
      // Use setTimeout to ensure the element is visible before scrolling (especially after display change)
      setTimeout(() => {
 ...
 function clearPlaceholdersForPreviousSelection(previousValue) {
  document
    ...
    .forEach((placeholder) => {
      // Ocultamos el placeholder anterior
